...
local function print_summary(pathname, options, print_state)
  ...
  if get_option('verbose', options, pathname) then
    ...
    if print_state.num_expl_tokens ~= nil then
      local num_expl_tokens = print_state.num_expl_tokens or 0
      table.insert(notes, string.format("%s tokens", format_num_expl_tokens(num_expl_tokens)))
    end
      ...
      if results.expl_ranges ~= nil then
        ...
        if num_total_bytes > 0 then
          ...
          if num_expl_bytes > 0 then
            ...
            local formatted_num_expl_tokens = string.format("%4s tokens", format_num_expl_tokens(num_expl_tokens):sub(-4))
