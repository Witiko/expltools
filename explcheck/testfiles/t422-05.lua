...
assert(#issues.errors == 6)
assert(#issues.warnings == 0)

local expected_line_numbers = {{3, 4}, {5, 6}, {9, 10}, {11, 12}, {13, 14}, {15, 16}}
local expected_contexts = {"!(tl ~= seq)", "!(str ~= seq)", "!(clist ~= seq)", "!(prop ~= seq)", "!(intarray ~= seq)", "!(fparray ~= seq)"}
